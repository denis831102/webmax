<template>
  <el-dialog :modelValue="props.visible" :before-close="closeWindow" :title="form.title" width="500">
    <el-form :model="form">
  ...
  switch (setting.value.dialog["editBuyers"].initiator) {
    case "table_Buyer_edit": {
      ...
      if (response.data.isSuccesfull) {
        ...
        [buyer.name, buyer.cod, buyer.commit, buyer.typeM] = [form.name, form.cod, form.commit, form.typeM];
        emit("update:visible", false);
      } else {
